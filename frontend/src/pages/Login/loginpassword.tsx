...
const LoginPasswordPage: React.FC = () => {
  const [identifier, setIdentifier] = useState<string>(''); // 아이디 또는 이메일
  const [message, setMessage] = useState<string>('');
  const navigate = useNavigate();
  const { requestPasswordReset } = useAuthStore(); // useAuthStore에서 requestPasswordReset 함수 가져오기

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setMessage('');
    try {
      await requestPasswordReset(identifier); // useAuthStore의 requestPasswordReset 함수 호출
      ...
      setMessage(error.response?.data?.message || '비밀번호 재설정 요청에 실패했습니다. 아이디 또는 이메일을 다시 확인해주세요.');
    }
  };
    ...
    <section className="min-h-[60vh] flex items-center justify-center bg-gradient-to-br from-orange-50 via-pink-50 to-purple-50 select-none">
      ...
      >
        ...
        <p className="text-sm text-gray-600 mb-4 text-center">
          아이디 또는 가입 시 사용한 이메일을 입력하시면, 비밀번호 재설정 링크를 보내드립니다.
        </p>
        <input
          type="text" // 아이디 또는 이메일
          name="identifier"
